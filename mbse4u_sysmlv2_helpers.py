...
def _check_response(response: requests.Response, endpoint: str = "") -> None:
    """Raise a typed exception for non-2xx HTTP responses.

    Mirrors the error-handling logic in the Open-MBEE SysMLV2Client._request.

    Args:
        response (requests.Response): The HTTP response to inspect.
        endpoint (str, optional): The URL or endpoint label for error messages.

    Raises:
        SysMLV2AuthError: On HTTP 401 or 403.
        SysMLV2NotFoundError: On HTTP 404.
        SysMLV2BadRequestError: On HTTP 400.
        SysMLV2ConflictError: On HTTP 409.
        SysMLV2APIError: On any other non-2xx status code.
    """
    code = response.status_code
    if code in (401, 403):
        raise SysMLV2AuthError(
            f"Authentication failed ({code}) for {endpoint}: {response.text}"
        )
    if code == 404:
        raise SysMLV2NotFoundError(
            f"Resource not found at {endpoint}: {response.text}"
        )
    if code == 400:
        try:
        ...
        raise SysMLV2BadRequestError(
            f"Bad request for {endpoint}: {details}"
        )
    if code == 409:
        raise SysMLV2ConflictError(
            f"Conflict at {endpoint}: {response.text}"
        )
    if not (200 <= code < 300):
        raise SysMLV2APIError(
            status_code=code,
            message=f"Unexpected status for {endpoint}. Response: {response.text}",
        )

def check_specialization_hierarchy(query_url: str, element: Dict[str, Any], super_element: Dict[str, Any], visited: set = None) -> bool:
    """
    Recursively checks if an element specializes the given super_element.
    ...
    print(f"Checking ownedSpecializations for {element.get('name')}: {element.get('ownedSpecialization')}")
...
def find_elements_specializing(server_url: str, project_id: str, commit_id: str, elements: List[Dict[str, Any]], super_element_name: str, element_kind: str = None) -> List[Dict[str, Any]]:
    ...
    if not super_element:
        print(f"Could not find super element: {super_element_name}")
        return []   
...
def get_commits(server_url: str, project_id: str) -> List[Dict[str, Any]]:
    ...
    _check_response(response, commits_url)
...
def get_element_documentation(server_url: str, project_id: str, commit_id: str, element_id: str, includeDefinition: bool = False) -> Optional[str]:
    """
    ...
    if element:
        element_doc_ids = element.get('documentation') 
        if element_doc_ids:
            bodies = []
            for ref in element_doc_ids:
            ...
            return bodies
        elif includeDefinition:
            for elementDefinition in element.get('definition'):
                definition = get_element_fromAPI(get_commit_url(server_url, project_id, commit_id), elementDefinition['@id'])
                if definition:
                    doc = get_element_documentation(server_url, project_id, commit_id, definition['@id'])
                    return doc
...
def get_element_fromAPI(query_url: str, element_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        _check_response(elements_response, url)
...
def get_elements_byKind_fromAPI(server_url: str, project_id: str, commit_id: str, kind: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        List[Dict[str, Any]]: List of matching element dictionaries.
    """
    print(f"get_elements_byKind_fromAPI called with server_url: {server_url}, project_id: {project_id}, commit_id: {commit_id}, kind: {kind}")
    query_url = f"{server_url}/projects/{project_id}/query-results?commitId={commit_id}"
...
def get_feature(server_url: str, project_id: str, commit_id: str, owner: Dict[str, Any], feature_name: str, feature_kind: str = 'AttributeUsage') -> Union[str, int, float, None]:
    """
    Retrieves a specific feature (by name and kind) from an element, including inherited features.
    ...
    Returns:
        Union[str, int, float, None]: The feature element, or None if not found.
    """
...
def get_metadata_ids_by_name(server_url: str, project_id: str, commit_id: str, metadata_shortnames: List[str]) -> Dict[str, str]:
    ...
    """
    print(f"get_metadata_ids_by_name called with server_url: {server_url}, project_id: {project_id}, commit_id: {commit_id}, metadata_shortnames: {metadata_shortnames}")
...
def get_owned_usages(server_url, project_id, commit_id, owner, feature_kind = None, feature_name = None, includeInherited = True):
    ...
    if not owner:
        return
    print(f"\n\nget_owned_usages called with owner: {owner.get('name')}, feature_kind: {feature_kind}")

    if not feature_name:
        feature_name = "feature"
    features = owner.get(feature_name, [])
    if not isinstance(features, list):
        features = [features]
    print(f"  Found {len(features)} {feature_kind} {feature_name} in owner {owner.get('name')}")
    print(f"  Features: {features}")
    feature_ids = [p.get('@id') for p in features]
    owned_features = get_elements_fromAPI(query_url, feature_ids)
    owned_usages = [p for p in owned_features if feature_kind and p.get("@type") == feature_kind]
    print(f"  Found {len(owned_usages)} {feature_kind} {feature_name} in owner {owner.get('name')}")
    
    if includeInherited:
        feature_ids = [p.get('@id') for p in owner.get("inheritedFeature", [])]
        inherited_usages = get_elements_fromAPI(query_url, feature_ids)
        for p in inherited_usages:
            print(f"  Found inherited feature: {p.get('declaredName')}, type: {p.get('@type')}") 
            if feature_kind and p.get("@type") == feature_kind and p.get("name") != None:
                owned_usages.append(p)
        print(f"  Found {len(owned_usages)} {feature_kind} in inherited features")
            
    return owned_usages

def get_owned_parameters(server_url, project_id, commit_id, owner, parameter_kind = "parameter"):
    """
    Fetches owned parameters for the owner from both Usage and Definition and adds them to the owner object.
    """

    query_url = get_commit_url(server_url, project_id, commit_id)

    print(f"\n\nget_owned_parameters called with owner: {owner.get('name')}, parameter_kind: {parameter_kind}")
    if not owner:
        return

    for p in owner.get(parameter_kind, []):
        print(f"  Found {p.get('declaredName')}, type: {p.get('@type')}")
    parameter_ids = [p.get('@id') for p in owner.get(parameter_kind, [])]
    owned_parameters = get_elements_fromAPI(query_url, parameter_ids)
    print(f"  Found {len(owned_parameters)} {parameter_kind} in owner")
            
    return owned_parameters
...
def get_projects(server_url: str, page_size: int = 256) -> List[Dict[str, Any]]:
    ...
    _check_response(response, projects_url)
...
def get_user_defined_concepts(server_url, project_id, commit_id, keyword: str, definition_kind: str, definition_name: str):
    """
    Retrieves user-defined concepts from the model.
    """
    metadata_definition_ids = get_metadata_ids_by_name(server_url, project_id, commit_id, [keyword])
    print(f"Found {len(metadata_definition_ids)} metadata definition IDs.")
    concept_ids = []
    if metadata_definition_ids:
        metadata_usages = get_metadatausage_annotatedElement_ids(server_url, project_id, commit_id, metadata_definition_ids)
        print(f"Found {len(metadata_usages)} metadata usages.")
        concept_ids = metadata_usages.get(keyword, [])
        print(f"Found {len(concept_ids)} concept IDs")
    if not concept_ids:
        definitions = get_elements_byKind_fromAPI(server_url, project_id, commit_id, definition_kind)
        print(f"Found {len(definitions)} {definition_kind}s")
        concept_ids = find_elements_specializing(server_url, project_id, commit_id, definitions, definition_name)               
        print(f"Found {len(concept_ids)} concept IDs")
...
def update_model_element(server_url, project_id, commit_id, element_id, feature_name, feature_value):
    ...
    Args:
        server_url (str): API Base URL.
        project_id (str): Project ID.
        commit_id (str): The current commit ID (used as previousCommit).
        element_id (str): The ID of the element to update.
        feature_name (str): The feature/property name to update.
        feature_value: The new value for the feature.

    Returns:
        str: The new commit ID after the update.
    """
    ...
    commit_post_response = session.post(commit_post_url, json=commit_body)
    _check_response(commit_post_response, commit_post_url)

    commit_response_json = commit_post_response.json()
    print(commit_response_json)
    new_commit_id = commit_response_json.get('@id', "")
